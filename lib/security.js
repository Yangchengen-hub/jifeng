...
function getBrowser(ua){
  ...
  if(/Chrome\/([\d.]+)/i.test(ua)){vm=ua.match(/Chrome\/([\d.]+)/i);v=vm?vm[1].split('.')[0]:'';return'Chrome'+(v?' '+v:'')}
  if(/Version\/([\d.]+).*Safari/i.test(ua)){vm=ua.match(/Version\/([\d.]+)/i);v=vm?vm[1].split('.')[0]:'';return'Safari'+(v?' '+v:'')}
  return'其他';
}
function getOS(ua){
  if(!ua)return'未知';
  ...
  if(/iPhone OS ([\d_]+)/i.test(ua)){var m=ua.match(/iPhone OS ([\d_]+)/i);return'iOS '+(m?m[1].replace(/_/g,'.'):'')}
