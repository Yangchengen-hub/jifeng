...
const ATTACK_PATTERNS=[
  ...
  {name:'NoSQL注入',regex:/(\$where|\$regex|\$ne|\$gt|\$lt|\$in|\$exists|ObjectId\(|\$expr|\$jsonSchema)/i},
  {name:'原型链污染',regex:/(__proto__|constructor\[|prototype\[|Object\.assign|\.\__proto__)/i},
  {name:'CRLF注入',regex:/%0d%0a|%0D%0A|\r\n(Set-Cookie|Location|Content-Type)/i},
  {name:'开放重定向',regex:/(redirect|url|next|return|returnTo|return_to)=https?:\/\/(?!jifeng)/i},
  {name:'模板注入',regex:/(\{\{.*\}\}|\$\{.*\}|<%.*%>|\{\%.*\%\}|\{#.*#\}|\{\{7\}\})/i},
  {name:'反序列化',regex:/(O:\d+:\"|java\.io\.|Serializable|readObject|unserialize|pickle\.loads|yaml\.load)/i},
  {name:'LDAP注入',regex:/(\*\|\(|\)\(|\(\|\(|\(&\(|\)\(\|\)|\*\(uid=|\(cn=|\(objectClass=)/i},
  {name:'XPath注入',regex:/(and\s+1=1|or\s+1=1|concat\(|substring\(|count\(\/|string-length\()/i},
  {name:'暴力破解',regex:/(password=.*password|user=.*admin.*pass|login.*brute|wp-login\.php\?action=register)/i},
  {name:'恶意文件上传',regex:/(\.php[3457s]?|\.phtml|\.phar|\.jsp|\.asp|\.aspx|\.cer|\.cgi|\.sh|\.bat|\.exe|\.dll|\.so|\.dylib)($|[?&\s])/i},
  {name:'漏洞扫描器',regex:/(nuclei|xray|awvs|appscan|webinspect|qualys|rapid7|tenable|openvas|nexpose|maxpatrol|positive|ptsecurity)/i},
  {name:'自动化工具',regex:/(curl\/|wget\/|httpie|postman|insomnia|paw\/|rest-client|swagger|openapi|api-client)/i},
  {name:'代理/隧道',regex:/(tor|anonymous|proxy|vpn|tunnel|relay|exit-node)/i}
];
